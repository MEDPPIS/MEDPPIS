...
def test(test_dataframe, psepos_path):
    ...
    for model_name in sorted(os.listdir(Model_Path)):
        print(model_name)
        model = MEDPPIS(LAYER, INPUT_DIM, HIDDEN_DIM, NUM_CLASSES, DROPOUT, LAMBDA, ALPHA)
        print(LAYER, INPUT_DIM, HIDDEN_DIM, NUM_CLASSES, DROPOUT)
        model.to(device)
        if torch.cuda.is_available():
            model.cuda()
...
def main():
    ...
    print("Evaluate MEDPPIS on Test_60")
    test_one_dataset(Test_60, Test60_psepos_Path)

    print("Evaluate MEDPPIS on Test_315-28")
    test_one_dataset(Test_315_28, Test315_28_psepos_Path)

    print("Evaluate MEDPPIS on UBtest_31-6")
